...
# %%
snekhorn = SNEkhorn(perp=perp, lr=1e-1, max_iter=500,
                    lr_sea=1e1, max_iter_sea=3000, init="random")
snekhorn.fit(X)
P_SE = snekhorn.PX_
...
fig, axs = plt.subplots(1, 4, figsize=(19, 4.5), constrained_layout=True)
...
axs[1].scatter(Z_sne[np.array(Y) == 0, 0], Z_sne[np.array(Y) == 0, 1],
               alpha=0.7, c='blue', edgecolor='k', label='$\mathbf{p}_1$')
axs[1].scatter(Z_sne[np.array(Y) == 1, 0], Z_sne[np.array(Y) == 1, 1],
               alpha=0.7, c='red', edgecolor='k', label='$\mathbf{p}_2$')
axs[1].set_title('Symmetric-SNE', fontsize=fs)
axs[1].set_xticks([])
axs[1].set_yticks([])

axs[3].scatter(Z_snekhorn[np.array(Y) == 0, 0], Z_snekhorn[np.array(
    Y) == 0, 1], alpha=0.7, c='blue', edgecolor='k', label='$\mathbf{p}_1$')
axs[3].scatter(Z_snekhorn[np.array(Y) == 1, 0], Z_snekhorn[np.array(
    Y) == 1, 1], alpha=0.7, c='red', edgecolor='k', label='$\mathbf{p}_2$')
axs[3].set_title('SNEkhorn', fontsize=fs)
axs[3].legend(loc='lower right', fontsize=fs-3)
axs[3].set_xticks([])
axs[3].set_yticks([])

plt.savefig('../data/saved/heteroscedastic_noise.pdf', bbox_inches='tight')
